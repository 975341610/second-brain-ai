...
export const NotionEditor: React.FC<NotionEditorProps> = ({
    ...
    editorProps: {
      ...
      handleDrop: (view, event, slice, moved) => {
        if (!moved && event.dataTransfer?.files?.[0]) {
          uploadLocalMedia(editorRef.current, event.dataTransfer.files[0]);
          return true;
        }
        return false;
      },
      handlePaste: (view, event) => {
        ...
        if (embed) {
          if (embed.kind === 'youtube') {
            ...
            editorRef.current?.commands.insertContent({ type: 'embedNode', attrs: { src: embed.src } });
            return true;
          }
        }
        return false;
  ...
  }, [extensions]); // Dependency on extensions to ensure Collaboration is updated when note changes

  // --- Utility Functions & Effect Handlers ---
  const updateTableRect = useCallback(() => {
    ...
    try {
      ...
      if (selection && selection.rangeCount > 0) {
        ...
        if (tableElement) {
          ...
          if (rect.width > 0 && rect.height > 0) {
            ...
            if (rowElement) setActiveRowRect(rowElement.getBoundingClientRect());
            else setActiveRowRect(null);
            return;
          }
        }
      }
      if (currentEditor.isActive('table')) {
        ...
        if (tableDOM) {
          setActiveTableRect(tableDOM.getBoundingClientRect());
          const rowDOM = currentEditor.view.dom.querySelector('tr:has(.selectedCell)');
          if (rowDOM) setActiveRowRect(rowDOM.getBoundingClientRect());
          else setActiveRowRect(null);
          return;
        }
      }
  ...
  const handleOutlineLeave = useCallback(() => {
    outlineTimeoutRef.current = window.setTimeout(() => setShowOutline(false), 300);
  }, []);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (tr) {
        ...
        if (tbody) rowIndex = Array.from(tbody.children).indexOf(tr);
      }
        ...
        <div className="flex flex-col w-full max-w-[800px] mx-auto">
          ...
          {viewMode === 'edit' && editor && activeTableRect && createPortal(
            ...
            >
              <div className="absolute top-0 bottom-0 -right-5 w-5 flex items-center justify-center pointer-events-auto">
                <button
                  onClick={(e) => {
                    ...
                    if (targetPos !== null) editor.chain().focus(targetPos + 1).addColumnAfter().run();
              ...
              <div className="absolute left-0 right-0 -bottom-5 h-5 flex items-center justify-center pointer-events-auto">
                <button
                  onClick={(e) => {
                    e.preventDefault();
                    const targetPos = activeRowPos ?? activeTablePos;
                    if (targetPos !== null) editor.chain().focus(targetPos + 1).addRowAfter().run();
              ...
              <div className="absolute -top-6 -left-6 w-6 h-6 flex items-center justify-center pointer-events-auto">
                <button
                  onClick={(e) => {
                    e.preventDefault();
                    const targetPos = activeTablePos;
                    if (targetPos !== null) editor.chain().focus(targetPos + 1).deleteTable().run();
              ...
              {activeRowRect && (
                <div 
                  className="row-side-handle absolute -left-[3px] w-[3px] pointer-events-auto transition-all cursor-pointer"
                  style={{
                    ...
                    >
                      <button
                        onClick={(e) => {
                          ...
                          if (targetPos !== null) editor.chain().focus(targetPos + 1).deleteRow().run();
              ...
              >
                <button
                  ...
                  onClick={(e) => {
                    if (viewMode !== 'edit') return;
                    try {
                      const pos = editor.view.posAtCoords({ left: e.clientX + 40, top: e.clientY });
                      if (pos && pos.inside !== -1) {
                        editor.chain().focus().setTextSelection(pos.pos).insertContent('/').run();
                      } else {
          ...
          {editor && (
            <BubbleMenu 
              editor={editor} 
              shouldShow={({ editor }) => viewMode === 'edit' && editor.isActive('table') && !editor.state.selection.empty}
            ...
            >
              <div className="flex items-center gap-0.5 border-r border-stone-200 pr-1 mr-1">
                <button onClick={() => editor.chain().focus().addRowBefore().run()} className="p-1.5 hover:bg-stone-100 rounded text-stone-600 transition-colors" title="在上方插入行">
                  <Rows size={16} className="rotate-180" />
                </button>
                <button onClick={() => editor.chain().focus().addRowAfter().run()} className="p-1.5 hover:bg-stone-100 rounded text-stone-600 transition-colors" title="在下方插入行">
                  <Rows size={16} />
                </button>
                <button onClick={() => editor.chain().focus().deleteRow().run()} className="p-1.5 hover:bg-red-50 rounded text-red-600 transition-colors" title="删除行">
                  <Trash2 size={16} />
                </button>
              </div>
              <div className="flex items-center gap-0.5 border-r border-stone-200 pr-1 mr-1">
                <button onClick={() => editor.chain().focus().addColumnBefore().run()} className="p-1.5 hover:bg-stone-100 rounded text-stone-600 transition-colors" title="在左侧插入列">
                  <Columns size={16} className="rotate-180" />
                </button>
                <button onClick={() => editor.chain().focus().addColumnAfter().run()} className="p-1.5 hover:bg-stone-100 rounded text-stone-600 transition-colors" title="在右侧插入列">
                  <Columns size={16} />
                </button>
                <button onClick={() => editor.chain().focus().deleteColumn().run()} className="p-1.5 hover:bg-red-50 rounded text-red-600 transition-colors" title="删除列">
                  <Trash2 size={16} />
                </button>
              </div>
              <div className="flex items-center gap-0.5 border-r border-stone-200 pr-1 mr-1">
                <button onClick={() => editor.chain().focus().mergeCells().run()} className="p-1.5 hover:bg-stone-100 rounded text-stone-600 transition-colors" title="合并单元格" disabled={!editor.can().mergeCells()}>
                  <Combine size={16} />
                </button>
                <button onClick={() => editor.chain().focus().splitCell().run()} className="p-1.5 hover:bg-stone-100 rounded text-stone-600 transition-colors" title="拆分单元格" disabled={!editor.can().splitCell()}>
                  <Split size={16} />
                </button>
              </div>
              <button onClick={() => editor.chain().focus().deleteTable().run()} className="p-1.5 hover:bg-red-100 rounded text-red-700 transition-colors ml-0.5" title="删除整个表格">
                <Trash2 size={16} />
              </button>
            </BubbleMenu>
          )}
          ...
          {viewMode === 'edit' && propertyMenuNode && editor && createPortal(
            ...
            >
              <div className="fixed inset-0 -z-10" onClick={() => setPropertyMenuNode(null)} />
              <div className="px-3 py-2 text-xs font-semibold text-stone-500 uppercase tracking-wider border-bottom border-stone-100">列操作</div>
              <div className="flex flex-col gap-0.5 mt-1 relative z-10">
                <button onClick={() => { editor.chain().focus(propertyMenuNode.pos + 1).addColumnBefore().run(); setPropertyMenuNode(null); }} className="flex items-center gap-2 px-3 py-2 text-sm text-stone-700 hover:bg-stone-100 rounded-md transition-colors w-full text-left">
                  <Columns size={16} className="text-stone-400 rotate-180" />
                  <span>左侧插入列</span>
                </button>
                <button onClick={() => { editor.chain().focus(propertyMenuNode.pos + 1).addColumnAfter().run(); setPropertyMenuNode(null); }} className="flex items-center gap-2 px-3 py-2 text-sm text-stone-700 hover:bg-stone-100 rounded-md transition-colors w-full text-left">
                  <Columns size={16} className="text-stone-400" />
                  <span>右侧插入列</span>
                </button>
                <div className="h-px bg-stone-100 my-1 mx-2" />
                <button onClick={() => { editor.chain().focus(propertyMenuNode.pos + 1).deleteColumn().run(); setPropertyMenuNode(null); }} className="flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-md transition-colors w-full text-left">
          ...
          {editor && (
            ...
            >
              <div className="flex items-center gap-0.5 p-1">
                ...
                <button onClick={() => editor.chain().focus().toggleItalic().run()} className={`p-1.5 rounded hover:bg-stone-100 ${editor.isActive('italic') ? 'text-blue-600 bg-blue-50' : 'text-stone-600'}`}>
                  <Italic size={16} />
                </button>
                <button onClick={() => editor.chain().focus().toggleUnderline().run()} className={`p-1.5 rounded hover:bg-stone-100 ${editor.isActive('underline') ? 'text-blue-600 bg-blue-50' : 'text-stone-600'}`}>
                  <Underline size={16} />
                </button>
                <button onClick={() => editor.chain().focus().toggleCode().run()} className={`p-1.5 rounded hover:bg-stone-100 ${editor.isActive('code') ? 'text-blue-600 bg-blue-50' : 'text-stone-600'}`}>
                  <Code size={16} />
                </button>
                <div className="w-px h-4 bg-stone-200 mx-1" />
                <button onClick={() => { const url = window.prompt('输入链接地址'); if (url) editor.chain().focus().setLink({ href: url }).run(); }} className={`p-1.5 rounded hover:bg-stone-100 ${editor.isActive('link') ? 'text-blue-600 bg-blue-50' : 'text-stone-600'}`}>
                  <Link2 size={16} />
                </button>
                <button onClick={() => handleAIAction('ask')} className="p-1.5 rounded hover:bg-stone-100 text-purple-600">
                  <Sparkles size={16} />
                </button>
                <div className="w-px h-4 bg-stone-200 mx-1" />
                <button onClick={() => editor.chain().focus().unsetAllMarks().run()} className="p-1.5 rounded hover:bg-stone-100 text-stone-600">
                  <Eraser size={16} />
                </button>
              </div>
            </BubbleMenu>
